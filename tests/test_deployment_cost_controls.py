...
def test_ci_and_ec2_deployment_only_auto_run_from_main() -> None:
    ...
    assert '.github/workflows/deploy-ec2.yml' in deploy_triggers
...
def test_ec2_cleanup_is_manual_opt_in_and_scoped() -> None:
    ...
    marker = "- name: Reclaim safe logs and caches before container recreation"
...
def test_mobile_runbook_blocks_duplicate_eas_builds() -> None:
    ...
    for contract in (
        ...
        "가드가 `pnpm run eas:build:preview` 직전에 같은 조회를 자동으로 다시 실행",
