...
| 2026-05-16 | INTRO2 | 아티스트 1열 나열 (1차) | `flex-row-layout`이 Tailwind `grid`를 cascade 덮어씀 | DynamicEngine.tsx grid/flex 키워드 감지 |
| 2026-05-16 | INTRO3 | 지역 chip 레이아웃 깨짐 (1차) | 동일 원인 | 동일 해결 |
| 2026-05-17 | INTRO2 | 아티스트 여전히 1열 나열 (2차, 근본 원인) | REPEATER가 각 아이템 div에 `grid grid-cols-3` 클래스를 적용 → 각자 독립 grid container가 되어 세로 1열 | DynamicEngine.tsx REPEATER grid wrapper 추가 |
| 2026-05-17 | INTRO3 | 지역 chip 여전히 1열 나열 (2차) | 동일 원인 | 동일 해결 |
| 2026-05-17 | INTRO2 | 아이돌 이미지 없음 | artistList `imageUrl` 빈 문자열, placeholder 파일도 없음 | CardImage.tsx 이니셜 폴백 처리 |
...
| INTRO2/3 grid 배치 | 프론트 재시작 후 3열/4열 확인 필요 | ⏳ 사용자 확인 필요 (코드 수정 완료 2026-05-17) |
...
DynamicEngine 변경 영향 범위 (2026-05-17 기준):
- `customClass`에 `grid`/`flex` 없는 GROUP → 기존과 동일 동작
- `customClass`에 `grid`/`flex` 있는 GROUP → direction 클래스 미추가 (CSS cascade 방지)
- `customClass`에 `grid` 있는 REPEATER → wrapper div 방식으로 변경 (INTRO2/3 해당)
- `customClass`에 `grid` 없는 REPEATER → 기존 방식 유지 (기존 화면 회귀 없음)
