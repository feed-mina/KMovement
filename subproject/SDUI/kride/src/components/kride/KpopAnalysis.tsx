...
export function UploadConsent({ meta, onAction, apiBase = '' }: LeafProps) {
  ...
  return (
    <section className="kpop-analysis-upload" aria-labelledby="kpop-analysis-upload-title">
      ...
      <label className="kpop-file-picker">
      ...
      {file ? (
        <div className="kpop-upload-preview">
          <img src={previewUrl} alt="분석할 의상 미리보기" />
          <div>
      ...
      <button type="button" className="kpop-primary-btn" disabled={!file || !consented || busy} onClick={submit}>
...
function CandidateCard({ candidate }: { candidate: KpopAnalysisCandidate }) {
  ...
  return (
    <article className="kpop-result-candidate">
      ...
      <h3>{candidate.name || '이름이 확인되지 않은 후보'}</h3>
  ...
  return (
    <div className="kpop-analysis-results">
      <div className="kpop-result-notice">
        <strong>{gradeCopy(grade)}</strong>
        <p>AI 결과는 비교를 시작하기 위한 후보입니다. 동일 상품·정품·구매 적합성을 보증하지 않습니다.</p>
        {typeof result.confidence === 'number' ? <p>모델 참고 점수 {Math.round(result.confidence)} / 100</p> : null}
      </div>
...
export function AiResultCard({ data, apiBase = '' }: LeafProps) {
  ...
  if (!jobId) return <section className="kpop-analysis-card"><p>분석 작업 번호가 없습니다.</p></section>;

  return (
    <section className="kpop-analysis-card" aria-live="polite">
      <div className="kpop-result-heading">
        <div>
          <p className="kpop-eyebrow">작업 #{jobId}</p>
          <h2>{statusCopy[job?.status || 'QUEUED']}</h2>
        </div>
        {!isKpopAnalysisTerminal(job?.status) ? (
          <small>{streamMode === 'polling' ? '자동 새로고침 중' : '실시간 상태 연결 중'}</small>
        ) : null}
      </div>
      {job && !isKpopAnalysisTerminal(job.status) ? (
        <div className="kpop-progress" aria-label={`분석 진행 ${job.progressPct || 0}%`}>
      ...
      <div className="kpop-result-actions">
        ...
        <button type="button" onClick={removeSource} disabled={Boolean(job?.sourceDeleted || job?.sourceDeletedAt) || deleting}>
