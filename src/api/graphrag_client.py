...
# 아티스트를 아예 고르지 않았거나, UI 목록 밖의 이름이 들어오거나, 자유
# 텍스트에서 아무 아티스트도 추출되지 않을 때 쓰는 경로다. 그때 아티스트 기반
# 확장이 0건이 되고 ChromaDB 도 비어 있으면 일정이 장소 없이 생성된다.
# 최소한 선택한 지역의 POI 는 돌려준다.
#
# UI 목록 32종은 전부 그래프에 있다. 예전에는 그래프 아티스트가 40종뿐이라
# 12종이 여기로 떨어졌는데, Supabase 의 성지 데이터를 병합해 137종이 된 뒤로는
# 그 이유로는 오지 않는다(#229, #231, #232).
